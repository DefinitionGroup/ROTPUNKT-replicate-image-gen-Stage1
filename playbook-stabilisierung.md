...
**Strenge in Phase 1.** `QUALITY_GATE_STRICTNESS` steuert, welche Checks im Enforce-Modus blockieren. Standard `counts`: nur Raumtyp und die Zählungen (eine Spüle, eine Armatur, ein Kochfeld) sind hart – das ist der Fehler vom 14. August und wird zuverlässig gefangen; Spülen-/Kochfeld-Ort und Inselanzahl werden geprüft und protokolliert, blockieren aber nicht, weil FLUX Inselzonen in 60–80 % der Fälle falsch platziert und das Gate sonst die meisten Inselaufträge abweisen würde. `full` macht alle Checks hart, sobald Reparatur oder Nachtraining die Platzierung tragen. Der Shadow Mode misst immer mit `full`, damit der Kalibrierungssatz vollständig bleibt; jeder Bericht speichert seine Strenge und die Liste der harten Checks.

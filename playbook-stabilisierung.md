...
**Strenge in Phase 1.** `QUALITY_GATE_STRICTNESS` steuert, welche Checks im Enforce-Modus blockieren – als Preset oder als Liste von Check-IDs. Standard `counts+camera`: Raumtyp, die Zählungen (eine Spüle, eine Armatur, ein Kochfeld) **und die gewählte Kameraperspektive** sind hart. Die Kamera kam am 3. September dazu, nachdem ein Enforce-Lauf (Satz `4d3957e9`) ein Augenhöhe-Bild für einen Vogelperspektiven-Auftrag geliefert hatte: Der perspektivisch richtige Kandidat war wegen Doppelbecken korrekt abgelehnt, der zweite bestand die Zählregeln und hatte Kamera, Kochfeld-Ort und Grifftyp falsch – alles weich. FLUX trifft die Vogelperspektive in 78 % der Batch-Bilder (25 von 32), deutlich zuverlässiger als die Inselaufteilung; die Perspektive hart zu machen kostet also wenig Trefferquote und ist für den Nutzer sofort sichtbar. Spülen-/Kochfeld-Ort, Inselanzahl und Grifftyp bleiben weich, bis Reparatur oder Nachtraining sie tragen. `counts` lässt die Kamera weich, `full` macht zusätzlich Ort, Insel und Grifftyp hart (Armaturenausrichtung und Griff-Sitz bleiben immer weich). Der Shadow Mode misst immer mit `full`; jeder Bericht speichert seine Strenge und die Liste der harten Checks.
